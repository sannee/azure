...
if( $config['files_allow'] == "yes" and $user_group[$member_id['user_group']]['allow_file_upload'] and in_array( strtolower( $type ), $allowed_files ) ) {
	...
	if( $area != "template" AND $user_group[$member_id['user_group']]['max_files'] ) {
		...
		if ($count_files AND $count_files >= $user_group[$member_id['user_group']]['max_files'] ) msg_error( "Upload Error", 403 );
	...
	if( $area != "template" AND $user_group[$member_id['user_group']]['max_images'] ) {
	...
	} elseif( $count_images AND $count_images >= $user_group[$member_id['user_group']]['max_images'] ) {
	...
	if( @move_uploaded_file( $image, $upload_path . $file_prefix . $image_name ) ) {
		...
		$i_info = @getimagesize($upload_path . $file_prefix . $image_name); 
		if( !in_array( $i_info[2], array (1, 2, 3 ) ) )	{
			@unlink( $upload_path . $file_prefix . $image_name );
			msg_error( "File is not an image", 403 );
		}
