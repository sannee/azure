...
while($row = $db->get_row()) {
	...
	$from = "<a onclick=\"javascript:popupedit('".urlencode($row['from'])."'); return(false)\" href=\"#\">{$row['from']}</a>";
	$to = "<a onclick=\"javascript:popupedit('".urlencode($row['to'])."'); return(false)\" href=\"#\">{$row['to']}</a>";
...
</select>&nbsp;<input class="edit" type="submit" value="{$lang['b_start']}"></div></th></tr>
</table>
...
$db->query("SELECT " . PREFIX . "_complaint.id, `c_id`, " . PREFIX . "_complaint.text, `from`, `to`, " . PREFIX . "_comments.autor, is_register, post_id, " . PREFIX . "_comments.text as c_text, " . PREFIX . "_post.title, " . PREFIX . "_post.date as newsdate, " . PREFIX . "_post.alt_name, " . PREFIX . "_post.category, " . PREFIX . "_post.flag  FROM " . PREFIX . "_complaint LEFT JOIN " . PREFIX . "_comments ON " . PREFIX . "_complaint.c_id=" . PREFIX . "_comments.id LEFT JOIN " . PREFIX . "_post ON " . PREFIX . "_comments.post_id=" . PREFIX . "_post.id WHERE c_id > '0' ORDER BY id DESC");
...
while($row = $db->get_row()) {
	...
	if($row['is_register'])
		$to = "<a onclick=\"javascript:popupedit('".urlencode($row['autor'])."'); return(false)\" href=\"#\">{$row['autor']}</a>";
	else $to = $row['autor'];
...
$db->query("SELECT " . PREFIX . "_complaint.id, `n_id`, " . PREFIX . "_complaint.text, `from`, `to`,  " . PREFIX . "_post.id as post_id, " . PREFIX . "_post.title, " . PREFIX . "_post.date as newsdate, " . PREFIX . "_post.alt_name, " . PREFIX . "_post.category, " . PREFIX . "_post.flag  FROM " . PREFIX . "_complaint LEFT JOIN " . PREFIX . "_post ON " . PREFIX . "_complaint.n_id=" . PREFIX . "_post.id WHERE n_id > '0' ORDER BY id DESC");
...
while($row = $db->get_row()) {
	...
	if (count(explode(".", $row['from'])) != 4 ) $from = "<a onclick=\"javascript:popupedit('".urlencode($row['from'])."'); return(false)\" href=\"#\">{$row['from']}</a>";
	else $from = $row['from'];
