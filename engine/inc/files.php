...
if( $_REQUEST['userdir'] ) $userdir = totranslit( $_REQUEST['userdir'], true, false ) . DIRECTORY_SEPARATOR; else $userdir = "";
if( $_REQUEST['sub_dir'] ) $sub_dir = totranslit( $_REQUEST['sub_dir'], true, false ) . DIRECTORY_SEPARATOR; else $sub_dir = "";

if( isset( $_REQUEST['area'] ) ) $area = totranslit( $_REQUEST['area'], true, false ); else $area = "";
...
JSCRIPT;
	...
	if( ! $wysiwyg ) {
		...
		echo "if (imageAlign == 'center') finalImage = \"<div style=\\\"text-align: center;\\\"><img src=\\\"\"+ selectedImage +\"\\\"></div><div></div>\";
		else finalImage = \"<img align=\\\"\"+ imageAlign +\"\\\" src=\\\"\"+ selectedImage +\"\\\">\";";
	}
...
if( $_POST['subaction'] == "deluploads" ) {
	...
	if( count( $_POST['static_files'] ) ) {
		...
		if ($user_group[$member_id['user_group']]['allow_admin']) $db->query( "INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('".$db->safesql($member_id['name'])."', '{$_TIME}', '{$_IP}', '33', '{$news_id}')" );
...
if( $_POST['subaction'] == "upload" ) {
	...
	for($image_i = 1; $image_i < ($images_number + 1); $image_i ++) {
		...
		if ( $serverfile != '' ) {
			...
			if( stripos ( $serverfile, "phtm" ) !== false ) die("Hacking attempt!");
			if( stripos ( $serverfile, "shtm" ) !== false ) die("Hacking attempt!");
		...
		if( $imageurl != "" ) {
			...
			if( $image_name != "" ) {
				...
				$image_name = totranslit( implode( ".", $img_name_arr ), false ) . "." . $type;
			
			}

			if( stripos ( $image_name, "php" ) !== false ) die("Hacking attempt!");
			if( stripos ( $image_name, "phtm" ) !== false ) die("Hacking attempt!");
			if( stripos ( $image_name, "shtm" ) !== false ) die("Hacking attempt!");
		...
		if( $action == "quick" AND $area != "template" AND $user_group[$member_id['user_group']]['max_images'] AND in_array( $type, $allowed_extensions ) ) {
		...
		if( $action == "quick" AND $area != "template" AND $user_group[$member_id['user_group']]['allow_file_upload'] AND $user_group[$member_id['user_group']]['max_files'] AND in_array( $type, $allowed_files ) ) {
		
			$row = $db->super_query( "SELECT COUNT(*) as count  FROM " . PREFIX . "_files WHERE author = '$author' AND news_id = '$news_id'" );
			$count_files = $row['count'];

			if ($count_files AND $count_files >= $user_group[$member_id['user_group']]['max_files'] ) $user_group[$member_id['user_group']]['allow_file_upload'] = false;

		} else $count_files = false;
...
*/
			...
			if( @file_exists( ROOT_DIR . "/uploads/files/" . $file_prefix . $image_name ) ) {
				...
				if( intval( $config['max_file_size'] ) and @filesize( ROOT_DIR . "/uploads/files/" . $file_prefix . $image_name ) > ($config['max_file_size'] * 1024) ) {
					...
					if ($user_group[$member_id['user_group']]['allow_admin']) $db->query( "INSERT INTO " . USERPREFIX . "_admin_logs (name, date, ip, action, extras) values ('".$db->safesql($member_id['name'])."', '{$_TIME}', '{$_IP}', '36', '{$file_prefix}{$image_name}')" );
		...
		} elseif( $user_group[$member_id['user_group']]['max_files'] AND $count_files AND $count_files >= $user_group[$member_id['user_group']]['max_files'] ) {

			$img_result .= "<div><font color=red>$current_image -> $lang[error_max_files]</font></div>";
		...
		} elseif( $user_group[$member_id['user_group']]['max_images'] AND $count_images AND $count_images >= $user_group[$member_id['user_group']]['max_images'] ) {

			$img_result .= "<div><font color=red>$current_image -> $lang[error_max_images]</font></div>";
...
<div>{$lang['images_upurl']}&nbsp;<input class="edit bk" type="text" name="imageurl" size=42></div>
<div class="hr_line"></div>
HTML;
...
if( $member_id['user_group'] == 1 ) {
	...
	echo <<<HTML
<div><b>uploads/files/</b>&nbsp;&nbsp;&nbsp;<input class="edit bk" type="text" name="serverfile" size=42></div>
<div class="hr_line"></div>
HTML;
...
HTML;
	...
	if( $area != "template" ) {
		...
		while ( $row = $db->get_row() ) {
			...
			$file_link = "<a class=maintitle href=\"javascript:insertfile('[attachment={$row['id']}:{$row['name']}]')\">{$row['name']}</a>";
