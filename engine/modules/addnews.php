...
if( ! $allow_addnews ) {
	...
	if( isset( $_REQUEST['mod'] ) and $_REQUEST['mod'] == "addnews" and $is_logged and $user_group[$member_id['user_group']]['allow_adds'] ) {
		...
		if ($config['create_catalog']) $catalog_url = totranslit($title{0}); else $catalog_url = "";
		...
		if( $config['allow_multi_category'] ) {
			...
			$cats = "<select data-placeholder=\"{$lang['addnews_cat_sel']}\" name=\"catlist[]\" id=\"category\" onchange=\"onCategoryChange(this)\" style=\"width:350px;height:140px;\" multiple>";
