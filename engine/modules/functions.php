...
function dle_cache($prefix, $cache_id = false, $member_prefix = false) {
	...
	if ( $mcache ) {
		...
		return memcache_get( $mcache, md5( DBNAME . PREFIX . $key ) );
...
function create_cache($prefix, $cache_text, $cache_id = false, $member_prefix = false) {
	...
	if ( $mcache ) {
		...
		memcache_set( $mcache, md5( DBNAME . PREFIX . $key ), $cache_text, MEMCACHE_COMPRESSED, 86400 );
...
function build_js($js, $config) {
	...
	if ($config['js_min'] AND version_compare(PHP_VERSION, '5.1.0', '>') ) {
		...
		$js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;g=general&amp;6\"></script>";

		if ( count($js) ) $js_array[] = "<script type=\"text/javascript\" src=\"{$config['http_home_url']}engine/classes/min/index.php?charset={$config['charset']}&amp;f=".implode(",", $js)."&amp;6\"></script>";
